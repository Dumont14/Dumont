...
export function MetarPanel({ icao }: MetarPanelProps) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/metar?icao=${icao}`)
      .then(r => r.json())
      .then(d => { if (d.error) setError(d.error); else setRaw(d.metar); })
  ...
  const badgeEl = cat
    ? <Badge label={cat} variant={CAT_VARIANT[cat]} />
    : null;
  ...
  return (
    <Panel
      ...
      badge={badgeEl}
    >
      {loading && <div className={styles.loading}><span className="spin" /> Fetching METAR…</div>}
